...
final class QMPVMHostSleepController: VMHostSleepControlling {
    ...
    init(socketPath: String) throws {
        let factory: ConnectionFactory = {
            try QMPConnection(
                ...
                identifierPrefix: "bento-host-power"
            )
        }
    ...
    func pauseIfRunning() throws -> Bool {
        ...
        throw lastError ?? BentoIntegrationError.io("QMP pause failed")
    }
    ...
    private func pauseOnFreshConnection() throws -> Bool {
        ...
        guard let running = status["running"] as? Bool else {
            throw BentoIntegrationError.io("QMP query-status response omitted the VM run state")
        }
    ...
    func resume() throws {
            ...
            do {
                ...
                if !running {
                    guard let runState = status["status"] as? String else {
                        throw BentoIntegrationError.io("QMP query-status response omitted the VM status")
                    }
        ...
        throw lastError ?? BentoIntegrationError.io("QMP resume failed")
    }
