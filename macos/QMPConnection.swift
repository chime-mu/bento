...
final class QMPConnection: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "dev.bento.qmp-connection", qos: .userInteractive)
    ...
    func executeCapturingEvents(
        ...
        try queue.sync { [self] in
            guard !closed else {
                throw BentoIntegrationError.io("QMP control is unavailable")
            }
            ...
            do {
                ...
                guard let matched = try Self.readResponse(
                    ...
                    throw BentoIntegrationError.io("QMP command \(command) timed out")
                }
                let response = matched.object
                if let error = response["error"] as? [String: Any] {
                    let detail = error["desc"] as? String ?? "unknown QMP error"
                    throw BentoIntegrationError.io("QMP command \(command) failed: \(detail)")
                }
                guard let result = response["return"] as? [String: Any] else {
                    throw BentoIntegrationError.io("QMP command \(command) returned an invalid response")
                }
    ...
    private func negotiateCapabilities(timeoutMilliseconds: Int32) throws {
        guard let greeting = try Self.readJSONObject(
            ...
            throw BentoIntegrationError.io("QMP socket did not send a valid greeting")
        }
        ...
        guard let matched = try Self.readResponse(
            ...
            throw BentoIntegrationError.io("QMP capability negotiation failed")
        }
    }
    ...
    private static func connectSecureSocket(path: String) throws -> Int32 {
        guard path.hasPrefix("/"), !path.utf8.contains(0) else {
            throw BentoIntegrationError.io("QMP socket path must be an absolute pathname")
        }
        let url = URL(fileURLWithPath: path).standardizedFileURL
        guard url.path == path else {
            throw BentoIntegrationError.io("QMP socket path must already be standardized")
        }
        ...
        guard lstat(parent.path, &parentInfo) == 0,
            ...
            throw BentoIntegrationError.io("QMP socket parent must be a private directory owned by this user")
        }
        var socketInfo = stat()
        guard lstat(path, &socketInfo) == 0,
              (socketInfo.st_mode & S_IFMT) == S_IFSOCK,
              socketInfo.st_uid == getuid() else {
            throw BentoIntegrationError.io("QMP endpoint must be a Unix socket owned by this user")
        }

        let pathBytes = Array(path.utf8)
        var address = sockaddr_un()
        guard pathBytes.count < MemoryLayout.size(ofValue: address.sun_path) else {
            throw BentoIntegrationError.io("QMP socket path is too long")
        }
        ...
        guard fileDescriptor >= 0 else { throw BentoIntegrationError.io("cannot create QMP socket") }
        ...
        guard withUnsafePointer(to: &noSignal, {
            ...
            throw BentoIntegrationError.io("cannot make QMP socket resilient to guest exit")
        }
        ...
        guard result == 0 else {
            ...
            Darwin.close(fileDescriptor)
            throw BentoIntegrationError.io("cannot connect to QMP socket: \(detail)")
        }
        ...
        try data.withUnsafeBytes { rawBuffer in
            ...
            while written < rawBuffer.count {
                ...
                if result > 0 {
                    ...
                    throw BentoIntegrationError.io("cannot write QMP command")
                }
            }
        }
    }
    ...
    private static func readJSONObject(
        ...
        while bytes.count <= 1_048_576 {
            ...
            if ready < 0 {
                ...
                throw BentoIntegrationError.io("cannot poll QMP socket")
            }
            guard descriptorPoll.revents & Int16(POLLIN) != 0 else {
                throw BentoIntegrationError.io("QMP socket closed while awaiting a response")
            }
            var byte: UInt8 = 0
            ...
            if count == 1 {
                if byte == 0x0A {
                    guard let object = try JSONSerialization.jsonObject(with: bytes)
                        as? [String: Any] else {
                        throw BentoIntegrationError.io("QMP response is not a JSON object")
                    }
                    return object
                }
                ...
                throw BentoIntegrationError.io("cannot read QMP socket")
            }
        }
        throw BentoIntegrationError.io("QMP response exceeds one MiB")
    }
